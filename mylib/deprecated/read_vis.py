...
def remove_charbychar(mainstring ,string):                                                                                               #◈
    ...
    return mainstring                                                                                                                    #◈
                                                                                                                                         #◈
def readAquantity(hdf_dir, hdf_file, quantity, keys_avail, my_transpose,myquiet):                                                        #◈
    if quantity in keys_avail:                                                                                                           #◈
            if myquiet == 0 : print("Reading "+quantity+" in "+hdf_dir)                                                                  #◈
            out = trans(np.array(hdf_file.get(quantity, "f")), my_transpose)                                                             #◈
            return out                                                                                                                   #◈
    else:                                                                                                                                #◈
...
#◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈                                                                                             #◈
class read_vis:                                                                                                                          #◈
    def __init__(self, raw_feed, feed_range=None, variable=None, quiet=0, equil_feed=None, dt=10, direct_timelist=None):                 #◈
        ...
        meta = {'d':[dx,dy,dz], 'axes':[x,y,z], 'n':[nx,ny,nz]}                                                                          #◈
        # for i in meta2:                                                                                                                #◈
            # try:                                                                                                                       #◈
                # if int(i[2]) < 1000:                                                                                                   #◈
                    # nx = int(i[1])                                                                                                     #◈
        ...
        avail_equillist = ['bx0','by0','bz0','te0','rho0','pe0']                                                                         #◈
        vector_checklist = [']','[']                                                                                                     #◈
        ...
        while True:                                                                                                                      #◈
            ...
            if isinstance(self.variable, str):                                                                                           #◈
                ...
                if equil_cond:                                                                                                           #◈
                    ...
                    with h5py.File(self.equil_feed, 'r') as hdf:                                                                         #◈
                        for mem in equil_list:                                                                                           #◈
                            if self.quiet == 0:print("Reading "+mem+"0 ...")                                                             #◈
                            equil_ss[mem+'0'] = trans(np.array(hdf.get(mem, "f")), transpose)                                            #◈
                else:                                                                                                                    #◈
                    pass                                                                                                                 #◈
                                                                                                                                         #◈
            ...
            for item in tobe_readlist:                                                                                                      #◈
                ...
                if accompany_tobe_readlist_formula[item_inx]:                                                                               #◈
                    ...
                    for jtem in readlist_item_nonequil:                                                                                     #◈
                        if jtem not in item_dic.keys():item_dic[jtem]=readAquantity(snapshot_dir,snapshot_h5,jtem,keys,transpose,self.quiet)#◈                              
                ...
                # If not a formula but contains equil quantities                                                                            #◈
                    ...
                    if item not in item_dic.keys():                                                                                         #◈
                        temp[item] = readAquantity(snapshot_dir,snapshot_h5,item,keys,transpose,self.quiet)                                 #◈
                    else:                                                                                                                   #◈
...
#                                            ◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈◈                                                                          
    ...
    def metadata(self):      # using a function's head identical to a var's name present in the same
        return self.meta     # namespace, i.e., within a class, would cause a namespace conflict
        
    def timelist(self):
        return self.timelist

    def keys(self):
        return list(self.ss[0].keys())
        
    def numfile(self):
        return len(self.fid1)
        
        
    def contour(self, numfig, frames, var, axes, point, rand=None, direc=os.getcwd(), varlbl = None, hidden = 0):
        self.n = numfig
        self.f = frames
        self.var = var
        self.axes = axes
        ...
        if self.axes == 'xy':
            axislst = [x,y]
            label = ['x (Mm)','y (Mm)', "Contourplots of "+self.var+" at z = "+str(self.point)]
            for item in self.ss.keys():
                dic[item] = np.transpose(self.ss[item][self.var][:,:,self.point])
        elif self.axes == 'xz':
            axislst = [x,z]
            label = ['x (Mm)','z (Mm)', "Contourplots of "+self.var+" at y = "+str(self.point)]
            for item in self.ss.keys():
                dic[item] = np.transpose(self.ss[item][self.var][:,self.point,:])
        else:
            axislst = [y,z]
            label = ['y (Mm)','z (Mm)', "Contourplots of "+self.var+" at x = "+str(self.point)]
            for item in self.ss.keys():
                dic[item] = np.transpose(self.ss[item][self.var][self.point,:,:])
        fig = {} 
        if self.quiet == 0 :print("Number of data files = ", len(self.fid1))
        ...
        # The case with only 1 files, hence two figures
            ...
            fig, axs = plt.subplots(1, 1, sharex=True, figsize=(20, 12), dpi=80, facecolor='w', edgecolor='k')
            pcm = axs.contourf(axislst[0], axislst[1], dic[0][:,:])
            #axs.set_title("t = "+str(self.rng[0]))
            axs.set_xlabel(label[0])
            axs.set_ylabel(label[1])
            axs.set_title("t = "+str(str(self.timelist[0])), fontsize = 16)
